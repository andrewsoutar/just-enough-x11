...
(defun parse-structlike (elem &optional extra-handler)
  ...
  (labels ((parse-switch-case (switch-case)
             (let* ((kind (or (find (dom:tag-name switch-case) '(:case :bitcase) :key #'string-downcase :test #'equal)
                              (error 'unknown-tag-error :element switch-case)))
                    (match-expressions (make-collector))
                    (fields (parse-structlike switch-case (lambda (elem)
                                                            (collect match-expressions (parse-expression elem))))))
               `(,kind :matches ,(collect match-expressions) :fields ,fields))))
    (mapcan (lambda (child)
                ...
                (cond ((equal tag "pad")
                      ...
                      ((equal tag "field")
                       (list `(:field :name ,(get-attribute-or-lose child "name")
                                      ...
                                      ,@(when-let ((attr (or (dom:get-attribute-node child "enum")
                                                             (dom:get-attribute-node child "mask"))))
                                          `(:enum (,(dom:value attr)
                                                   ,(when (equal (dom:node-name attr) "mask") :mask)))))))
                      ((equal tag "switch")
                       (let ((cases (child-elems child)))
                         (list `(:switch-struct :name ,(get-attribute-or-lose child "name")
                                                :expr ,(parse-expression (pop cases))
                                                :cases ,(mapcar #'parse-switch-case cases)))))
                      ((and extra-handler (funcall extra-handler child))
                       ())
                      (t (error 'unknown-tag-error :element child)))))
            (child-elems elem))))
