               ...
               (match-ecase parameters
                 ((lone-var)
                  (multiple-value-bind (param-type value)
                      (match-let* (((('type type (m:eql lone-var))) (collect (parameter-decls inner-info)))
                                   (enum-alist
                                    (when enum
                                      (mapcar (lambda (entry)
                        ...
                        (flet ((parse-one-enum-value (var)
                          ...
                          (cond (maskp
                                 (assert (eql (car type) 'unsigned-byte))
                                 (values 'list `(reduce #'logior ,var :key (lambda (x) ,(parse-one-enum-value 'x)))))
                                (enum
                                 (assert (member (car type) '(unsigned-byte signed-byte)))
                                 (values `(or ,type (member ,@(mapcar #'car enum-alist))) (parse-one-enum-value var)))
                                (t (values type var)))))
